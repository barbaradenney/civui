...
describe('civds-form', () => {
  ...
  it('fires civds-invalid on invalid submit', async () => {
    ...
    el.addEventListener('civds-invalid', ((e: CustomEvent) => {
  ...
  describe('analytics', () => {
    it('fires civds-analytics with submit action on valid submission', async () => {
      const el = createFixture(`
        <civds-form>
          <civds-text-input label="Name" name="name" value="John"></civds-text-input>
          <button type="submit">Submit</button>
      ...
      expect(submitCall[0].detail.componentName).toBe('civds-form');
    });

    it('fires civds-analytics with invalid action on validation failure', async () => {
      const el = createFixture(`
        <civds-form>
          <civds-text-input label="Name" name="name" required></civds-text-input>
          <button type="submit">Submit</button>
    ...
    it('suppresses analytics when disable-analytics is set', async () => {
      ...
      const formAnalytics = handler.mock.calls.filter(
        (c: any) => c[0].detail.componentName === 'civds-form',
      );
    ...
    it('never includes form data in analytics payload', async () => {
      const el = createFixture(`
        <civds-form>
          <civds-text-input label="Name" name="name" value="John Doe"></civds-text-input>
          <button type="submit">Submit</button>
        </civds-form>
      `) as any;
      await waitForChildren(el);
