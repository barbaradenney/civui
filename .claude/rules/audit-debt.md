...
- **Superseded note (2026-06-01):** the "Was"/"Fix" text above describes the pre-re-base state — at the time the `spacing` scale was 5px-based (`civ-p-12` = 60px), so decoupling `width` onto its own 4px scale was the fix. On **2026-06-01 the whole `spacing` scale was re-based to 4px** (see `.claude/rules/spacing.md` → "Modular scale" / re-base note), so `civ-p-12` is now **48px** and the spacing and width scales coincide for these keys. The dedicated `theme.extend.width` block is now redundant for parity (spacing would match anyway) but remains as an explicit, self-documenting guard and is still covered by the regression test — keep it.
...
### Colored `civ-link-card` variants lack hover / active feedback — ✅ shipped 2026-05-31

- **Was:** When a consumer set both `color` and `variant="primary"` on `<civ-link-card>`, the renderer swapped the interactive `.civ-link-card--primary` class for the static `.civ-card--{color}-primary` class, which had no `:hover` / `:active` rules — so the card didn't respond to mouse interaction despite being a real clickable affordance. The `ColorsPrimary` Storybook story is the documented placement.
- **Fix:** Added one scoped rule in `components.css` (after the `.civ-card--{color}-primary` block) — `.civ-link-card[class*='civ-card--'][class*='-primary']` with `filter: brightness(0.95)` on `:hover` (under `@media (hover: hover)`) and `brightness(0.9)` on `:active`, plus a `filter` transition on the motion tokens. Scoping to `.civ-link-card` keeps the static `civ-card` chrome flat. `filter: brightness()` darkens the composited pixels uniformly across all eight categorical colors and works in both light and dark mode (it operates on the rendered output, not the token) — so one rule covers every `tag-{color}-bg` pairing without 8 × 2 new hover-bg tokens. The global `prefers-reduced-motion` rule in `civ.css` disables the transition. No new tokens, no schema change.
