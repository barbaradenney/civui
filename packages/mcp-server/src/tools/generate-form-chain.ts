...
export function generateFormChain(
  ...
  const jsLines: string[] = [
    ...
    '  }',
    '',
    '  // Build a step list-item without innerHTML so a form label',
    '  // containing HTML (e.g. an onerror img) renders as text, not markup.',
    '  // Only the decorative icon + visually-hidden status span use trusted',
    '  // static strings; the label always goes through a text node.',
    '  function renderStep(li, step, state) {',
    '    li.textContent = "";',
    '    if (state === "completed" || state === "locked") {',
    '      var icon = document.createElement("span");',
    '      icon.setAttribute("aria-hidden", "true");',
    '      icon.className = state === "completed" ? "civ-text-success" : "civ-text-base-dark";',
    '      icon.textContent = state === "completed" ? "\\u2713" : "\\uD83D\\uDD12";',
    '      li.appendChild(icon);',
    '      li.appendChild(document.createTextNode(" " + step.label + " "));',
    '      var sr = document.createElement("span");',
    '      sr.className = "civ-sr-only";',
    '      sr.textContent = state === "completed" ? "(completed)" : "(locked)";',
    '      li.appendChild(sr);',
    '    } else if (state === "current") {',
    '      var cur = document.createElement("span");',
    '      cur.setAttribute("aria-current", "step");',
    '      cur.className = "civ-font-bold";',
    '      cur.textContent = step.label;',
    '      li.appendChild(cur);',
    '    } else {',
    ...
    '      if (completed) {',
    '        renderStep(li, step, "completed");',
    '      } else if (current) {',
    '        renderStep(li, step, "current");',
    '      } else if (locked) {',
    '        renderStep(li, step, "locked");',
    '      } else {',
    '        renderStep(li, step, "default");',
    '      }',
    '    });',
    '',
    '    // Update back button',
