...
| `<civ-accordion>` | Layout | `single` (one-open-at-a-time), `disabled` (cascades to items). Methods: `expandAll()`, `collapseAll()`. | — |
...
7. **Don't add a focus-ring class to interactive elements.** `civ.css` applies the focus indicator globally to every `button` / `a[href]` / `input` / `select` / `textarea` / `[role="button"]` / `[tabindex]:not([tabindex="-1"])`. Both `focus:civ-outline-*` and the legacy `focus-visible:civ-focus-ring` per-element class are deprecated.
