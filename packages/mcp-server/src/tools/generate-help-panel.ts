...
function generateSidebar(sections: HelpSection[]): { html: string; js: string } {
  ...
  htmlParts.push(
    `<button type="button" data-civ-help-toggle class="civ-px-3 civ-py-1 civ-bg-primary civ-text-white civ-rounded focus-visible:civ-focus-ring" aria-expanded="false" aria-controls="civ-help-sidebar">Help</button>`,
  );
  htmlParts.push(
...
function generateInline(sections: HelpSection[]): { html: string; js: string } {
  ...
  for (const section of sections) {
    ...
    htmlParts.push(
      ...
      `  <button type="button" id="${escapeHtml(linkId)}" data-civ-help-expand="${escapeHtml(section.id)}" class="civ-text-primary civ-underline civ-text-sm focus-visible:civ-focus-ring" aria-expanded="false" aria-controls="${escapeHtml(contentId)}">More info: ${escapeHtml(section.heading)}</button>`,
...
function generateTooltip(sections: HelpSection[]): { html: string; js: string } {
  ...
  for (const section of sections) {
    ...
    htmlParts.push(
      ...
      `  <button type="button" data-civ-help-trigger="${escapeHtml(section.id)}" aria-describedby="${escapeHtml(tooltipId)}" class="civ-rounded-full civ-w-5 civ-h-5 civ-bg-info civ-text-white civ-text-xs civ-inline-flex civ-items-center civ-justify-center focus-visible:civ-focus-ring">?</button>`,
