...
const schema: ComponentSchema = {
  $schema: '1.0',
  name: 'civ-routing-number',
  description: 'Pre-configured text input for bank routing-number entry. Ships with the `routing` validator (9-digit mod-10 checksum), 9-character maxlength, numeric keyboard, and the plain-language default label/hint. Used on direct-deposit flows. Thin wrapper over `civ-text-input` — same event surface (`civ-input`, `civ-change` with `{ value: string }`).',
