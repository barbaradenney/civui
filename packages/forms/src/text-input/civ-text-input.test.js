...
describe('civ-text-input', () => {
    ...
    it('renders hint text', async () => {
        const el = createFixture('<civ-text-input label="Email" hint="Enter your work email"></civ-text-input>');
        await waitForUpdate(el);
    ...
    it('renders error message with alert role', async () => {
        const el = createFixture('<civ-text-input label="Email" error="Email is required"></civ-text-input>');
        await waitForUpdate(el);
    ...
    it('sets aria-invalid when error is present', async () => {
        const el = createFixture('<civ-text-input label="Email" error="Invalid"></civ-text-input>');
        await waitForUpdate(el);
        const input = el.querySelector('input');
    ...
    it('sets aria-describedby for hint and error', async () => {
        const el = createFixture('<civ-text-input label="Email" hint="Hint text" error="Error text"></civ-text-input>');
        await waitForUpdate(el);
        const input = el.querySelector('input');
    ...
    it('renders with different input types', async () => {
        const el = createFixture('<civ-text-input label="Password" type="password"></civ-text-input>');
        await waitForUpdate(el);
        const input = el.querySelector('input');
        expect(input.type).toBe('password');
    });
    it('renders disabled state', async () => {
        const el = createFixture('<civ-text-input label="Email" disabled></civ-text-input>');
        await waitForUpdate(el);
        const input = el.querySelector('input');
        expect(input.disabled).toBe(true);
    });
    it('fires civ-input event on input', async () => {
        const el = createFixture('<civ-text-input label="Email" name="email"></civ-text-input>');
        await waitForUpdate(el);
        const input = el.querySelector('input');
    ...
    it('renders with placeholder', async () => {
        const el = createFixture('<civ-text-input label="Email" placeholder="you@example.com"></civ-text-input>');
        await waitForUpdate(el);
        const input = el.querySelector('input');
    ...
    it('applies error border classes when error is set', async () => {
        const el = createFixture('<civ-text-input label="Email" error="Required"></civ-text-input>');
        await waitForUpdate(el);
        const input = el.querySelector('input');
    ...
    it('applies width variant classes', async () => {
        const el = createFixture('<civ-text-input label="ZIP" width="sm"></civ-text-input>');
        await waitForUpdate(el);
        const input = el.querySelector('input');
        expect(input.className).toContain('civ-w-24');
    });
    it('has static formAssociated = true', () => {
        const Ctor = customElements.get('civ-text-input');
        expect(Ctor.formAssociated).toBe(true);
    });
});
//# sourceMappingURL=civ-text-input.test.js.map