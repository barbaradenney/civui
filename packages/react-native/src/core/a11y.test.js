...
describe('buildAccessibilityLabel', () => {
    ...
    it('adds required to label', () => {
        expect(buildAccessibilityLabel({ label: 'Email', required: true })).toBe('Email, required');
    });
    it('combines label and hint', () => {
        expect(buildAccessibilityLabel({ label: 'Name', hint: 'Enter your name' })).toBe('Name. Enter your name');
    });
    it('combines label and error', () => {
        expect(buildAccessibilityLabel({ label: 'Email', error: 'Required' })).toBe('Email. Error: Required');
    });
    it('combines all parts', () => {
        expect(buildAccessibilityLabel({
            label: 'Email',
            hint: 'Work email',
            error: 'Invalid',
            required: true,
