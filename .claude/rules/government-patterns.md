...
- Error text uses `civ-text-error` (6.98:1 on white — AA). For success status text at body size, use `civ-text-success-dark` (4.63:1 on white — AA), not `civ-text-success` (which is 3.14:1 — AA-large/non-text only). For warning status text, use `civ-text-warning-darkest` on a `warning-lightest` background (7.05:1 — AAA), not bare `civ-text-warning` (`warning-DEFAULT` is 1.84:1 — fails everything).
