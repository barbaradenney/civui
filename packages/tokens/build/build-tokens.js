...
function buildTypeScript(tokens, scalesJs, darkTokens) {
  const obj = resolveTokenTree(tokens, (token) => toCSSValue(token));
  const lines = ['// Auto-generated by @civui/tokens — do not edit', ''];

  // NOTE: emit plain JS (no `as const`) — this file is shipped as
  // `.js` and imported at runtime by `@civui/mcp-server` and any other
  // consumer. The companion `tokens.d.ts` declares the TS shapes.
  lines.push(`export const tokens = ${JSON.stringify(obj, null, 2)};`);
  lines.push('');
  if (darkTokens) {
    // Dark palette overrides — only `color.*` paths populated (matches
    // color-dark.tokens.json scope). Consumers needing dark-mode color
    // resolution import `darkTokens` and walk the same `.color.<family>.<shade>`
    // path they used on `tokens`. Non-color categories (spacing,
    // typography, etc.) don't have a dark variant and live only on `tokens`.
    const darkObj = resolveTokenTree(darkTokens, (token) => toCSSValue(token));
    lines.push(`export const darkTokens = ${JSON.stringify(darkObj, null, 2)};`);
    lines.push('');
  }
  if (scalesJs) {
    lines.push(`export const scales = ${JSON.stringify(scalesJs, null, 2)};`);
    lines.push('');
  }
  // Runtime JS — no TS types. The `TokenPath` alias and the typed
  // `token()` signature live in the companion `.d.ts`. Keeping the
  // emitted .js parser-clean lets bundlers (rollup / vite / esbuild)
  // load it without a TS transform — required for the @civui/mcp-server
  // import path that brings these tokens into the contrast checker.
  lines.push(
...
export declare const darkTokens: Record<string, any>;
...
function build() {
  ...
  // TypeScript — tokens + scales + dark tokens export
  writeFileSync(join(distDir, 'js', 'tokens.js'), buildTypeScript(tokens, scalesJs, darkTokens));
