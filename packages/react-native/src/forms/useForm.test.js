...
describe('useForm validation logic', () => {
    ...
    it('passes when required fields have values', () => {
        const errors = validateFields({ name: 'John', email: 'john@test.com' }, {
            name: { label: 'Name', required: true },
            email: { label: 'Email', required: true },
        });
    ...
    it('runs custom validation', () => {
        const errors = validateFields({ email: 'bad-email' }, {
            email: {
                label: 'Email',
    ...
    it('passes custom validation', () => {
        const errors = validateFields({ email: 'john@test.com' }, {
            email: {
                label: 'Email',
                validate: (v) => (v.includes('@') ? undefined : 'Must be valid email'),
            },
    ...
    it('uses field name when label is missing', () => {
        const errors = validateFields({}, { phone: { required: true } });
        expect(errors.phone).toBe('phone is required');
    });
    it('handles multiple fields with mixed results', () => {
        const errors = validateFields({ name: 'John', email: 'bad', phone: '' }, {
            name: { label: 'Name', required: true },
            email: {
                label: 'Email',
    ...
    it('treats whitespace-only values as empty for required', () => {
        const errors = validateFields({ name: '   ' }, { name: { label: 'Name', required: true } });
        expect(errors.name).toBe('Name is required');
    });
});
describe('useForm data collection logic', () => {
    function collectFormData(values) {
        const data = {};
