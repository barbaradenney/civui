...
These are the proposed work items, ordered by visible-quality leverage per line of code. Most have shipped in branch `claude/typography-system-design-RykOU`; the remaining open items are scoped at the end.

1. ~~**Leading trim on headings + body**~~ — shipped 2026-05-27 (commit `01b7e5d`). Extended `text-box-trim: both; text-box-edge: cap alphabetic` to all `.civ-heading-{xl|lg|md|sm}` + `-secondary` variants and added the `.civ-text-trim` utility class for one-off opt-ins (`packages/core/src/styles/components.css:635-645`). The existing trim on `.civ-label` / `.civ-legend` / `.civ-hint` / `.civ-data-field__value` was preserved. Result: every card, callout, modal, alert, notice, and section reads visually tighter — the visible glyph now aligns with the container's padding edge rather than rendering with the line-box's half-leading extending past it.
2. ~~**Tabular numerals on data surfaces and numeric inputs**~~ — shipped 2026-05-27 (commit `a7fe4d6`). Extended `font-variant-numeric: tabular-nums` to `.civ-metric-tile__value`, `.civ-timeline-item__timestamp` (and the relative variant), `.civ-pagination__status` / `__page`, and the `.civ-input` selector when `inputmode="numeric|decimal|tel"` or `type="number"` is set (covers SSN, EIN, ZIP, phone, currency, routing as a side effect of their input mode). Added the `.civ-num` utility (`components.css:647-665`). Existing opt-ins on data-grid / table / count / itemized-total were kept in their original sites for adjacency with text-align rules.
3. ~~**Text wrap balance/pretty**~~ — shipped 2026-05-27 (commit `a7fe4d6`, same change as #2). `text-wrap: balance` applied to all heading classes + `.civ-page-header__heading|__subheading` + `.civ-alert__heading` + `.civ-notice__header` + `.civ-link-card__heading` + `.civ-section-intro__heading` + `.civ-repeater__row-heading`. `text-wrap: pretty` applied to `.civ-text-body|muted|caption|small`, `.civ-hint` (and `--group` variant), `.civ-alert__body`, `.civ-notice__body`, `.civ-section-intro__body`, `.civ-link-card__description`. `components.css:667-708`. Tailwind utility classes `civ-text-balance` and `civ-text-pretty` provided for arbitrary opt-ins.
4. ~~**Underline geometry tokens**~~ — shipped 2026-05-27. Three new tokens in `typography.tokens.json` → `typography.underline`: `thickness` (0.0625em / 1px at body size), `thicknessHover` (0.125em / 2px), `offset` (0.1875em / 3px). Applied uniformly across `.civ-link--*` + `.civ-breadcrumb__link` (thickness + offset) and `.civ-data-grid__sort-btn:hover` (thicknessHover + offset). The remaining hardcoded values are on `.civ-nav__link` — its `:hover` and `--current` states both use `text-underline-offset: 4px` + `text-decoration-thickness: 3px`. This is a persistent state-indicator surface (active page) plus a hover preview of that same treatment, where the larger clearance and heavier bar read as "accent line under the label" rather than "this is hyperlinked." Hover deliberately mirrors the active-page geometry so the two states match. Annotated in `components.css` as a deliberate one-off; promote to tokens if a second state-indicator surface appears. Em-based geometry means a heading-sized link now gets a proportionally thicker underline at proportionally larger offset, which is what the eye expects.
5. ~~**Font-feature baseline on body**~~ — shipped 2026-05-27 (commit `6681cdb`). Added `font-feature-settings: 'kern' 1, 'liga' 1, 'calt' 1; font-kerning: normal; font-optical-sizing: auto;` to `body` in `civ.css:36`. OpenType feature defaults are now consistent across macOS / Windows / Android, and variable system fonts (SF Pro, Segoe UI Variable, Roboto Flex) pick the correct optical-size variant per rendered point size.
6. ~~**`.civ-prose` wrapper utility**~~ — shipped 2026-05-27 (commit `39ab102`). `.civ-prose { max-width: 65ch; text-wrap: pretty; hanging-punctuation: first allow-end last; }` lives at `components.css:734-738`. Wraps long-form content blocks at the WCAG 1.4.8 recommended 45–75 chars/line; bundles orphan prevention and Safari-only hanging-punctuation polish in one class.
7. ~~**Print typography audit**~~ — shipped 2026-05-27 (commit `f15b23e`). Added `@media print` font sizes for `.civ-text-body` (11pt), `.civ-text-caption` (9pt), `.civ-text-small` (8pt), `.civ-heading-xl|xl-secondary` (14pt), `.civ-heading-lg|lg-secondary` (13pt), `.civ-heading-md|md-secondary` (12pt), and the existing `.civ-heading-sm|sm-secondary` (11pt) + `.civ-eyebrow` (8pt with letter-spacing preserved). `civ.css:235-`. Government forms now print at paper-calibrated sizes rather than screen-calibrated rem values.
