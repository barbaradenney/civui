...
## Chip family follow-ups (action-chip + input-chip)

- **Surfaced:** Chip family expansion, 2026-05-25. Branch `claude/component-library-audit-D5MR8`. Originally deferred at the Callout component landing (2026-05-23) "until a real placement appears"; user asked for the speculative slice anyway as audit slice 6.4.
- **What landed:** Two new sibling components in `@civui/actions` — `<civ-action-chip>` (fire-and-forget rounded chip button; single `<button>` element; `civ-click` event; no toggle state, no check icon) and `<civ-input-chip>` (display chip representing user-entered content with an always-present remove handle; `civ-remove` event; chip body is non-interactive, only the × is). `<civ-filter-chip>` keeps its existing toggle behavior unchanged — three sibling components, not one polymorphic one. CSS refactored to share a `.civ-chip` base + `.civ-chip--filter` / `.civ-chip--action` / `.civ-chip--input` modifiers so all three reuse the radius / border / hover / focus / disabled rules. Schemas, tests (~25 new), Storybook stories, and Docusaurus pages all ship in the same change.
- **Naming note (still valid):** If someone proposes renaming `civ-filter-chip` → `civ-chip` with a `role` / `chip-role` prop, push back: it forces every consumer to thread a prop to distinguish behaviors that are semantically different, and the runtime cost (validation, conditional rendering of the check icon and remove button) is higher than three small components. The siblings already share the BEM base; the user-visible API stays simple per-component.
- **Native + Drupal stubs (parity coverage) deferred.** Schemas exist for `civ-action-chip` and `civ-input-chip` and they validate cleanly + produce Props/Events partials, but neither is registered in `tools/schema-parity.ts` `COVERED_COMPONENTS`. To register: add iOS Swift stubs (`packages/ios/Sources/CivUI/Civ{ActionChip,InputChip}.swift`), Android Kotlin stubs (`packages/android/src/main/kotlin/gov/civui/components/Civ{ActionChip,InputChip}.kt`), and Drupal SDC YAMLs (`packages/drupal/civui/components/{action-chip,input-chip}/{name}.component.yml`). Pattern: shape `civ-action-chip` after `civ-button` (single interactive button host, `civ-click` event); shape `civ-input-chip` after `civ-filter-chip` (non-interactive wrapper + nested remove button, `civ-remove` event) — except input-chip has no toggle state, so the iOS/Android binding is simpler (just label + remove callback).
- **Why deferred:** Same rationale as the existing native-stubs entry — focus ring behavior on the wrapper-with-nested-button pattern needs device verification (iOS UIFocus, Android FocusableInTouchMode), and the keyboard-tab-stop expectations for an "X only" interactive chip differ enough between platforms that a blind implementation would risk shipping a broken tab order. Schemas being in place keeps the contract stable for whoever picks this up.
