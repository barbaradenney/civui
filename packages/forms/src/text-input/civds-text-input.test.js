...
describe('civds-text-input', () => {
    ...
    it('renders hint text', async () => {
        const el = createFixture('<civds-text-input label="Email" hint="Enter your work email"></civds-text-input>');
        await waitForUpdate(el);
    ...
    it('renders error message with alert role', async () => {
        const el = createFixture('<civds-text-input label="Email" error="Email is required"></civds-text-input>');
        await waitForUpdate(el);
    ...
    it('sets aria-invalid when error is present', async () => {
        const el = createFixture('<civds-text-input label="Email" error="Invalid"></civds-text-input>');
        await waitForUpdate(el);
        const input = el.querySelector('input');
    ...
    it('sets aria-describedby for hint and error', async () => {
        const el = createFixture('<civds-text-input label="Email" hint="Hint text" error="Error text"></civds-text-input>');
        await waitForUpdate(el);
        const input = el.querySelector('input');
    ...
    it('renders with different input types', async () => {
        const el = createFixture('<civds-text-input label="Password" type="password"></civds-text-input>');
        await waitForUpdate(el);
        const input = el.querySelector('input');
        expect(input.type).toBe('password');
    });
    it('renders disabled state', async () => {
        const el = createFixture('<civds-text-input label="Email" disabled></civds-text-input>');
        await waitForUpdate(el);
        const input = el.querySelector('input');
        expect(input.disabled).toBe(true);
    });
    it('fires civds-input event on input', async () => {
        const el = createFixture('<civds-text-input label="Email" name="email"></civds-text-input>');
        await waitForUpdate(el);
        const input = el.querySelector('input');
    ...
    it('renders with placeholder', async () => {
        const el = createFixture('<civds-text-input label="Email" placeholder="you@example.com"></civds-text-input>');
        await waitForUpdate(el);
        const input = el.querySelector('input');
    ...
    it('applies error border classes when error is set', async () => {
        const el = createFixture('<civds-text-input label="Email" error="Required"></civds-text-input>');
        await waitForUpdate(el);
        const input = el.querySelector('input');
    ...
    it('applies width variant classes', async () => {
        const el = createFixture('<civds-text-input label="ZIP" width="sm"></civds-text-input>');
        await waitForUpdate(el);
        const input = el.querySelector('input');
        expect(input.className).toContain('civds-w-24');
    });
    it('has static formAssociated = true', () => {
        const Ctor = customElements.get('civds-text-input');
        expect(Ctor.formAssociated).toBe(true);
    });
});
//# sourceMappingURL=civds-text-input.test.js.map