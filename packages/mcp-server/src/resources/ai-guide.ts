...
| \`<civ-button>\` | Action | \`label\`, \`emphasis\` (primary/secondary/tertiary), \`danger\`, \`type\`, \`disabled\`, \`loading\`, \`loading-label\`, \`icon-start\`, \`icon-end\`, \`icon-only\`, \`href\` (renders as link with button chrome) | \`civ-analytics\` |
| \`<civ-action-button>\` | Action | \`label\`, \`emphasis\` (primary/secondary/tertiary, default tertiary), \`spacing\` (default/sm), \`danger\`, \`disabled\`, \`loading\`, \`loading-label\`, \`pressed\` (tri-state toggle), \`current\` (aria-current="page"), \`href\`, \`icon-start\`, \`icon-end\`, \`icon-only\`. Compact button for toolbars and row actions. | \`civ-analytics\` |
| \`<civ-text-button>\` | Action | \`label\`, \`emphasis\` (primary/secondary/tertiary, default secondary), \`spacing\` (default/sm), \`disabled\`, \`loading\`, \`loading-label\`, \`icon-start\`, \`icon-end\`. Small inline click affordance — the text-btn primitive. | \`civ-click\`, \`civ-analytics\` |
| \`<civ-confirm-button>\` | Action | \`label\`, \`success-label\`, \`success-ms\`, \`emphasis\` (primary/secondary/tertiary), \`disabled\`. Fire-and-forget action with a transient ✓ receipt. | \`civ-confirm\`, \`civ-analytics\` |
| \`<civ-toggle-button>\` | Action | \`label\`, \`pressed-label\`, \`pressed\`, \`emphasis\` (primary/secondary/tertiary), \`icon-start\`, \`disabled\`. Two-state press toggle with \`aria-pressed\`. | \`civ-toggle\`, \`civ-analytics\` |
| \`<civ-filter-chip>\` | Action | \`label\`, \`value\`, \`selected\`, \`disabled\`, \`emphasis\` (primary/secondary — selected-state weight), \`variant\` (toggle/radio — set automatically by group in single mode), \`spacing\` (default/sm/lg), \`icon-start\`, \`icon-end\`, \`count\`. Toggle-only — for removable user-entered tokens use \`<civ-input-chip>\`. | \`civ-change\`, \`civ-analytics\` |
| \`<civ-action-chip>\` | Action | \`label\`, \`value\`, \`disabled\`, \`spacing\` (default/sm/lg), \`icon-start\`, \`icon-end\`, \`count\`. Fire-and-forget rounded chip (no toggle state, no check icon). | \`civ-click\`, \`civ-analytics\` |
| \`<civ-input-chip>\` | Action | \`label\`, \`value\`, \`disabled\`, \`spacing\` (default/sm/lg). User-entered token with always-present × remove handle. | \`civ-remove\`, \`civ-analytics\` |
