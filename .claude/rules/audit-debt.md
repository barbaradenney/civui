...
- **What did NOT land — four deferred items:**

  1. **Asymmetric `error` ladder.** `warning/success/info` restructured to put the softest pale surface at `lightest`. `error` did not — its softest surface is still `error.lighter` (#f4e3db), and the new `error.lightest` (#faf0f0) was added but is not consumed by any CSS rule. The lint:semantic-color-recipe RECIPE special-cases error (`secondary` bg uses `error-lighter` while the other three intents use `*-lightest`); same in lint:contrast PAIRS. Why deferred: normalizing error to match siblings means migrating every CSS rule that uses `error-lighter` (form-error-summary, alerts, badges, counts, cards, danger buttons) plus picking a new mid-tone value for the freed `error-lighter` slot — essentially repeating the warning/success/info restructure for error. Worth doing in a focused PR; current asymmetry is documented in 4 places (RECIPE docstring, lint-contrast comment, design-rules.md table footnote, audit-debt here).
  2. **`check-contrast.ts` is a hand-maintained mirror of `color.tokens.json`.** The MCP tool at `packages/mcp-server/src/tools/check-contrast.ts` exposes a `TOKEN_COLORS` object with ~30 hex values that mirror the canonical JSON. Updated in lockstep during every palette pass; no test asserts the mirror stays correct. Also light-mode-only — agents calling `check_contrast` for dark-mode validation get light-mode hex. The deeper fix: have it import the generated `packages/tokens/dist/js/tokens.js` (which already has the same shape) and flatten at module load. Both modes can be exposed via a mode argument or by exporting both palettes.
  3. **Tailwind configs (root + `apps/civ-site`) duplicate the color block.** ~95% identical — only `content`, the `surface` color block, and the focus-ring plugin differ. Every token-shape change requires touching 3 files (JSON + 2 configs) instead of 1. The generated `packages/tokens/dist/tailwind/preset.js` already produces the same shape. Migration: `apps/civ-site/tailwind.config.ts` can `import baseConfig from '../../tailwind.config'` and merge, or both can import the preset.
  4. **`lint:semantic-color-recipe` scope.** RECIPE only governs `.civ-badge` and `.civ-count` selectors. The new alert/card/process-list/timeline-item rules that migrated from `*-lighter` to `*-lightest` (`components.css` lines 4068–4071, 5121–5129, 5286, 6021–6024) are not gated — a refactor renaming `*-lightest` back to `*-lighter` "for brevity" would silently re-introduce the new (darker) mid-tone visual. Extend RECIPE to cover semantically-intent-typed soft-bg rules across the system, OR factor those rules to compose a shared CSS custom property the recipe lint checks centrally.

- **What to watch for in the meantime:** When adding a new intent-typed surface to a component, copy the `*-lightest` bg + `*-darkest` text pattern (or `error-lighter` + `error-dark` for error). Don't reach for `*-lighter` as a bg paired with `*-dark` text — `success-lighter + success-dark = 3.32:1` and `warning-lighter + warning-dark = 3.92:1` both fail AA body. The new mid-tone is for `*-lighter` + `*-darkest` pairings (success: 7.10, warning: 6.73, info: 8.20 — all AAA).
