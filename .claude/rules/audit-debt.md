...
| `civ-notice` — `font-size: 2.5rem` (default icon), `1.5rem` (sm icon) | ~~Replace with `var(--civ-typography-fontSize-2xl)` / `fontSize-lg`~~ **Done** in `claude/scale-tokens-tier3`. | Default icon now reads `fontSize-2xl` (40px → 24px, 16px shrink — matches `civ-alert` icon size), sm icon reads `fontSize-lg` (24px → 18px). Both scale with page typography. |
| `civ-count` — repeated `min-width: 1.25rem; line-height: 1.25rem;` (default) and `1rem` (sm) | ~~Move to `var(--civ-spacing-*)` equivalents~~ **Done** in `claude/scale-tokens-tier3`. | Default → `var(--civ-spacing-4)` (20px, exact match), sm → `var(--civ-spacing-3)` (16px → 15px, 1px shrink). Applied to both `--style-secondary` and `--style-primary` variants. |
| `civ-filter-chip` — `min-height: 1.75rem` (default), `1.5rem` (`--sm`) | ~~Move to `var(--civ-spacing-6)` / `--civ-spacing-5`~~ **Done** in `claude/scale-tokens-tier3` with a WCAG-floor twist. | Using `min-height: max(var(--civ-spacing-N), 1.5rem)`. The straight `--civ-spacing-5` / `--civ-spacing-6` recommendation would have shrunk to 18.75px / 22.5px under `data-civ-scale="dense"` (`spacingFactor: 0.75`) — below the WCAG SC 2.5.8 Target Size (Minimum) floor of 24px. `max(..., 1.5rem)` clamps the floor while preserving scale-aware upper range. **Pattern for future tokenization of touch-target dimensions**: use `max()` with a WCAG-floor literal whenever the spacing token can dip below 24px under dense scale. |
