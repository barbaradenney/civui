...
3. **`lint:between-block-cadence` (proposed, not built today)** — gates the rule that Tier 2 components (fieldset, card, alert, form-error-summary, input-group, page-header--sm, filterable-list__filters) all have `civ-mb-4` set in their component CSS. Tier 3 components don't get the lint; the catalog is hand-maintained at the top of the lint file. Catches the canonical drift mode: a new Tier 2 component lands without `mb-4`, breaking the stack rhythm. ~80 LOC + helper tests; would land in a follow-up branch.
