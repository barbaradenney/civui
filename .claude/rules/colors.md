...
- `warning` carries a unique `ink` shade (#1b1b1b) that — unlike every other shade — is **non-inverting**: it holds the same value in light AND dark mode. It exists for dark text on the always-yellow `warning-light` surface (`civ-link-card--critical`), which stays yellow in both modes. Don't treat `ink` as part of the lightness ladder; it's a special-purpose text token. See "Token hygiene" below and the resolved audit-debt entry "Non-inverting dark-text token for warning-light surfaces."
...
12. ~~**Hardcoded-hex lint**~~ — shipped 2026-05-29 as `pnpm lint:hardcoded-hex`. Parses `components.css` / `civ.css` into rule blocks, strips comments + `var(…)` spans (so defensive `var(--token, #fallback)` fallbacks pass), and fails on any remaining bare hex literal. Two escape hatches: an `ALLOWLIST_CLASSES` entry (a deliberate, documented literal) and a `/* hex ok: <reason> */` annotation on the declaration or the line above. The repo is clean today and `ALLOWLIST_CLASSES` is now empty — the former `civ-link-card--critical` literal moved to the non-inverting `warning-ink` token on 2026-05-29 (see "Token hygiene" above). 13 helper tests (`scanCss` takes an injectable allowlist so the mechanism stays covered with an empty prod list); wired into `pnpm validate:lints` and the drift-lints CI gate.
